      ...
      for(var i = 0;i<per_num;i++)//每页显示的页数。如何调整每页显示的内容刚好对应相应的俄页面
        {
          ...
          pageContent+='<li>';
          pageContent+='<div class="item item1"><p class="ID-link">ID: <a href="#"id="TargetID">'+data[per_num*(current_page-1)+i].TargetID+'</a></p></div>';
          pageContent+='<div class="item item2"><p><a href="#"id="ProteinName">'+data[per_num*(current_page-1)+i].ProteinName+'</a></p><div class="chemical-member"><ul><li><p id="basic">Basic Information</p></li><li><p id="pathway">Pathway</p></li><li><p id="related">Related Compounds</p></li><li><p id="supporting">Supporting Information</p></li></ul></div></div>';
          pageContent+='<div class="item item3-t"><ul><li>UniprotID <span id="UniprotID">'+data[per_num*(current_page-1)+i].UniprotID+'</span></li><li>Organisml<span id="Organisml">'+data[per_num*(current_page-1)+i].Organisml+'</span></li><li>Sequencel<span id="Sequencel">'+data[per_num*(current_page-1)+i].Sequencel+'</span></li><li>PDBID <span id="PDBID">'+data[per_num*(current_page-1)+i].PDBID+'</span></li><li>PISMID <span id="PISMID">'+data[per_num*(current_page-1)+i].PISMID+'</span></li></ul></div>';
          pageContent+='</li>';
        }
      pageContent+='</ul>';
...
$.each(data,function(index,items){
  ...
  $(b[index]).children("ul").children("li").click(function(){
    ...
    $(b[index]).find("li:nth-child(1)").click(function(){
      $(b[index]).parent().next().empty().append('<ul><li>Organisml <span id="Organisml">'+items.Organisml+'</span></li><li>Sequencel <span id="Sequencel">'+items.Sequencel+'</span></li></ul>');

    });
    // $(b[index]).find("li:nth-child(2)").click(function(){
    //   $(b[index]).parent().next().empty().append('<ul><li>PDBID <span id="PDBID">'+items.PDBID+'</span></li><li>PISMID <span id="PISMID">'+items.PISMID+'</span></li><li>Uniprot ID <span id="UniprotId">'+items.pathway.uniportID+'</span></li><li>Gene <span id="gene">'+items.pathway.gene+'</span></li><li>Function <span id="function">'+items.pathway.Function+'</span></li></ul>');
    // });
    // $(b[index]).find("li:nth-child(3)").click(function(){
    //   $(b[index]).parent().next().empty().append('<ul><li>Protein <span id="Protein">'+items.related.protein+'</span></li><li>Organism <span id="Organism">'+items.related.organism+'</span></li><li>Uniprot ID <span id="UniprotId">'+items.related.uniportID+'</span></li><li>Gene <span id="gene">'+items.related.gene+'</span></li><li>Function <span id="function">'+items.related.Function+'</span></li></ul>');
    // });
    // $(b[index]).find("li:nth-child(4)").click(function(){
    //   $(b[index]).parent().next().empty().append('<ul><li>Protein <span id="Protein">'+items.supporting.protein+'</span></li><li>Organism <span id="Organism">'+items.supporting.organism+'</span></li><li>Uniprot ID <span id="UniprotId">'+items.supporting.uniportID+'</span></li><li>Gene <span id="gene">'+items.supporting.gene+'</span></li><li>Function <span id="function">'+items.supporting.Function+'</span></li></ul>');
    // });
    $(".item3-t").find("span").mouseover(function(){
